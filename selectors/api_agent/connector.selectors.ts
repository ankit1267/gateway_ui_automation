export const ConnectorSelectors = {
  // navigation
  connectorsTab: 'role=tab[name="Connectors"]',

  // add tool / agent
  addFirstToolButton: 'embed-list-add-tool-button-empty',
  addFirstAgentButton: 'connected-agent-list-add-agent-button-empty',
  addFirstKnowledgeBase: 'knowledgebase-add-button',

  addNewToolsText: 'text=Add new Tools',
  suggestionDropdown: 'embed-suggestion-dropdown-menu',
  suggestionDropdownAgent:'#connect-agent-suggestion-dropdown',
  suggestionDropdownKB:'#knowledgebase-dropdown',

  // embed
  embedHeader: '#viasocket-embed-header',
  embedContainer: '#render-embed-container',
  agentContainer : '#connected-agent-list-agents-container'
};
